...
created=()
trap 'for id in "${created[@]+"${created[@]}"}"; do [ -n "$id" ] && curl "${A[@]}" -X DELETE "$U/work_packages/$id" 2>/dev/null; done; [ -n "${VS:-}" ] && RB "Version.where(id:[$VS]).destroy_all" 2>/dev/null; [ -n "${VN:-}" ] && RB "Version.where(id:[$VN]).destroy_all" 2>/dev/null' EXIT
